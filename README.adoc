...
Non il n’est pas  structuré en couches.
Le contrôleur contient à la fois la logique HTTP, l’accès aux données et une règle métier.
Cela pose des problèmes si on continue de faire évoluer l'application par la suite, le code est difficile à tester unitairement,
et la modification d’une partie peut impacter les autres.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans la méthode `finalContent()` de `TodoListController`.
Cette méthode applique la règle RG1 : si un item a plus de 24h, on ajoute `[LATE!]` devant son contenu.
La logique métier devrait être dans le domaine, pas dans la couche HTTP.

*Cette règle est-elle facilement testable par un test unitaire ?*
Non, pas facilement, car elle est privée et située directement dans le contrôleur.
Elle n’est pas isolée dans une classe métier dédiée.
...
- `-it` : mode interactif
- `-e POSTGRES_PASSWORD=password` : définit le mot de passe PostgreSQL
- `-p 5432:5432` : mappe le port PostgreSQL du conteneur sur la machine locale
- `postgres` : image utilisée

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
`todoItemRepository` était `null` car la classe possédait un constructeur vide.
Spring utilisait ce constructeur au lieu de celui qui injecte le repository.
La meilleure solution est de supprimer le constructeur vide et d’utiliser uniquement l’injection par constructeur.
...
L'architecture hexagonale permet d'isoler le code métier de l'infrastructure externe (base de données, API externe, …). 
Les grands principes de l’architecture sont :

- Isolation du Domaine : Le cœur de l'application ne dépend d'aucun framework technique extérieur.
- Système de Ports : Le domaine définit des interfaces (Ports) pour communiquer avec l'extérieur. Il y a des ports d'entrée et des ports de sortie
- Adaptateurs : Les classes techniques (REST, implémentation JPA) qui se branchent sur les ports pour faire le lien entre le monde extérieur et le domaine.
- Inversion des dépendances : Toutes les dépendances pointent toujours vers l'intérieur. Les adaptateurs dépendent du domaine, mais le domaine ne dépend jamais des adaptateurs.
...
Pour les adaptateurs, il faut écrire des tests d'intégration. Contrairement au domaine qui se teste avec de simples tests unitaires isolés, les adaptateurs interagissent avec l'extérieur (le framework Web, la BDD).

- Pour l'adaptateur REST (entrée), il faut tester le routage HTTP, la sérialisation/désérialisation JSON et les codes de retour.
- Pour l'adaptateur de persistance (sortie), il faut tester que les requêtes SQL (ou JPA) fonctionnent réellement contre une base de données.
